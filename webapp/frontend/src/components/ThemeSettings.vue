...
export default {
  data () {
    return {
      themeColor: Cookies.get('theme-primary'),
      sideBarOption: Cookies.get('theme-side'),
      colors: colors
    };
  },
  computed: {
  ...
  watch: {
    themeColor: {
      handler (val) {
        const base = this.colors[val] ? this.colors[val].base : this.colors["blue"]
        Cookies.set('theme-primary', val)
